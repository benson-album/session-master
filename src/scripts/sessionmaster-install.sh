...
if [ -z "$SERVER_JS" ] && command -v curl &>/dev/null; then
  ...
  info "从 GitHub 下载..."
  SERVER_JS=$(curl -fsSL "https://raw.githubusercontent.com/BenSongLab/session-master/main/src/server/server.js" 2>/dev/null) || true
fi
...
if [ -z "$SERVER_JS" ]; then
  ...
  info "或者从 GitHub 下载:"
  info "  curl -fsSL https://raw.githubusercontent.com/BenSongLab/session-master/main/src/server/server.js -o $INSTALL_DIR/server.js"
  exit 1
fi
