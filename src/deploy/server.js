...
const routes = {
  ...
  async pair(req, res) {
    ...
    if (!pair) pair = { key, createdAt: new Date().toISOString(), devices: {}, cookies: {} };
    pair.devices[deviceId] = { lastSeen: new Date().toISOString(), name: body.deviceName || deviceId };
    writePair(key, pair);
  ...
  async upload(req, res) {
    ...
    if (!key || !deviceId || !domain) return sendJSON(res, 400, { error: '缺少必要字段' });
  ...
  download(req, res) {
    ...
    if (!key || !deviceId) return sendJSON(res, 400, { error: '缺少参数' });
    const pair = readPair(key);
    if (!pair) return sendJSON(res, 404, { error: '配对码不存在' });
    const others = {};
    for (const [did, cookie] of Object.entries(pair.cookies)) if (did !== deviceId) others[did] = cookie;
  ...
  status(req, res) {
    ...
    if (!pair) return sendJSON(res, 404, { error: '配对码不存在' });
    sendJSON(res, 200, { key: pair.key, createdAt: pair.createdAt, deviceCount: Object.keys(pair.devices).length, devices: Object.entries(pair.devices).map(([id, info]) => ({ id: id.substring(0, 8) + '...', name: info.name, lastSeen: info.lastSeen })), cookieCount: Object.keys(pair.cookies).length });
  },
  ...
  async signalPoll(req, res) {
    ...
    sendJSON(res, 200, await waitForMessages());
  },
  ...
  signalLeave(req, res) {
    ...
    if (roomId && peerId) {
      ...
      if (room) {
        ...
        for (const [pid, q] of room.queues) q.push({ type: 'peer_left', from: peerId, timestamp: new Date().toISOString() });
...
const server = http.createServer(async (req, res) => {
  ...
  if (req.method === 'OPTIONS') {
    res.writeHead(204, { 'Access-Control-Allow-Origin': '*', 'Access-Control-Allow-Methods': 'GET, POST, OPTIONS, DELETE', 'Access-Control-Allow-Headers': 'Content-Type' });
    return res.end();
  }

  try {
    if (pathname === '/api/health' && req.method === 'GET') routes.health(req, res);
    else if (pathname === '/api/pair' && req.method === 'POST') await routes.pair(req, res);
    else if (pathname === '/api/sync/upload' && req.method === 'POST') await routes.upload(req, res);
    else if (pathname === '/api/sync/download' && req.method === 'GET') routes.download(req, res);
    else if (pathname === '/api/pair/status' && req.method === 'GET') routes.status(req, res);
    else if (pathname === '/api/signal/room' && req.method === 'POST') await routes.signalCreateRoom(req, res);
    else if (pathname === '/api/signal/message' && req.method === 'POST') await routes.signalSend(req, res);
    else if (pathname === '/api/signal/poll' && req.method === 'GET') await routes.signalPoll(req, res);
    else if (pathname === '/api/signal/room' && req.method === 'DELETE') routes.signalLeave(req, res);
    else {
      if (req.method === 'GET') {
        const pairCount = fs.readdirSync(DATA_DIR).filter(f => f.endsWith('.json')).length;
        res.writeHead(200, { 'Content-Type': 'text/html; charset=utf-8' });
        res.end(`<!DOCTYPE html><html><head><meta charset="utf-8"><title>SessionMaster Sync</title><style>body{font-family:sans-serif;max-width:600px;margin:50px auto;padding:20px}h1{color:#1a73e8}.status{background:#e8f0fe;padding:12px;border-radius:8px}</style></head><body><h1>🔐 SessionMaster Sync Server</h1><div class="status"><p>✅ 服务器运行中</p><p>⏰ ${new Date().toLocaleString()}</p><p>📊 ${pairCount} 个云同步配对 | P2P 信令服务运行中</p></div><p style="color:#888;margin-top:20px;font-size:14px">API: /api/pair | /api/sync/upload | /api/sync/download | /api/signal/room | /api/signal/message | /api/signal/poll | /api/health</p></body></html>`);
