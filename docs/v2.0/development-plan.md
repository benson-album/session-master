...
当 DE/QA/PD 发现问题影响整个批次的范围或交付物时触发。
...
##### 模式 G：架构变更重启（PM → PD → 用户 → 全 Agent）

当问题触及 PRD 层面的设计决策时触发，影响最大。
...
PM 收到任何 Agent 的异常报告后，按以下流程快速判断：
...
每个层级向上包含——🟠 的判断也包含 🟡 的评估，🔴 包含 🟠 的全部内容。
PM 不必降级处理，但可以升级（🟡→🟠→🔴 逐级上报）。
```
