...
| 层级 | 角色 | 代号 | 核心职责 | 产出物 | 签字权 |
|:----:|:----:|:----:|:---------|:-------|:------:|
| 总负责 | **Hermes 主代理** | `ME` | 方向设定、里程碑审查、PM 管理、用户决策升级 | 方法论、决策记录、里程碑终签 | ✅ 终签 |
| 执行层 | **项目管理** | `PM` | 任务调度、进度跟踪、日常决策、风险上报 | 任务看板、开发计划、里程碑报告 | ✅ 批次内 |
| 执行层 | **产品设计** | `PD` | PRD 维护、UI/UX 决策、方案评审、帮助文档 | PRD.md、UI 原型/说明 | — |
| 执行层 | **功能开发** | `DE` | 代码实现、模块拆分、bug 修复 | `src/*.js`、`*.html`、`*.css` | — |
| 执行层 | **功能测试** | `QA` | 测试执行、回归测试、缺陷报告 | test-plan.md、测试报告、bug 清单 | ✅ 质量门禁 |
