...
> ⚠️ 两份导出文件均为 53 个非 HttpOnly Cookie，无 `uin`/`skey` 等登录凭证。
> 微信登录（`main_login=wx`）产生的认证 Cookie 可能不在标准路径上，
> 或因过期导致导出时 Session 已失效。
> **退出保护验证需在用户真实登录的浏览器上测试**——插件安装后开启退出保护开关，
> 点击退出按钮即应触发拦截弹窗。若导入的 Cookie 不含登录凭证（HttpOnly 或特定域），页面处于未登录态，SDK 退出按钮不会出现，拦截自然不触发。
